...
const azure_arm_storage_1 = require("azure-arm-storage");
...
function promptForResource(ui, resourceType, resourcesTask) {
    return __awaiter(this, void 0, void 0, function* () {
        const picksTask = resourcesTask.then((resources) => {
            const picks = !util_1.isArray(resources) ? [] : (resources
                .map((r) => r.name ? { data: r, label: r.name } : undefined)
                .filter((p) => p));
            picks.push({
                label: localize_1.localize('skipForNow', '$(clock) Skip for now'),
                data: undefined,
        ...
        const result = (yield ui.showQuickPick(picksTask, options)).data;
        if (!result) {
            throw new errors_1.SkipForNowError();
        }
        else {
            return result;
        }
...
function promptForCosmosDBAccount() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const node = yield extensionVariables_1.ext.tree.showTreeItemPicker(vscode_azureextensionui_1.SubscriptionTreeItem.contextValue);
...
function promptForStorageAccount(actionContext, filterOptions) {
    return __awaiter(this, void 0, void 0, function* () {
        const node = yield extensionVariables_1.ext.tree.showTreeItemPicker(vscode_azureextensionui_1.SubscriptionTreeItem.contextValue);
        const wizardContext = Object.assign({}, node.root);
        ...
        const client = vscode_azureextensionui_1.createAzureClient(node.root, azure_arm_storage_1.StorageManagementClient);
...
function promptForServiceBus() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const node = yield extensionVariables_1.ext.tree.showTreeItemPicker(vscode_azureextensionui_1.SubscriptionTreeItem.contextValue);
        const client = vscode_azureextensionui_1.createAzureClient(node.root, azure_arm_sb_1.ServiceBusManagementClient);
