...
function validateFunctionProjects(actionContext, folders) {
            ...
            for (const folder of folders) {
                ...
                if (yield isFunctionProject(folderPath)) {
                    ...
                    if (isInitializedProject(folderPath)) {
                        ...
                        if (yield promptToInitializeProject(folderPath)) {
                            yield vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: localize_1.localize('creating', 'Initializing project...') }, () => __awaiter(this, void 0, void 0, function* () {
                                yield initProjectForVSCode_1.initProjectForVSCode(actionContext, folderPath);
                            }));
                            // don't wait
...
function promptToInitializeProject(folderPath) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (ProjectSettings_1.getFuncExtensionSetting(settingKey)) {
            ...
            const result = yield extensionVariables_1.ext.ui.showWarningMessage(message, vscode_azureextensionui_1.DialogResponses.yes, vscode_azureextensionui_1.DialogResponses.dontWarnAgain, vscode_azureextensionui_1.DialogResponses.learnMore);
            if (result === vscode_azureextensionui_1.DialogResponses.dontWarnAgain) {
                ...
                return yield promptToInitializeProject(folderPath);
            }
            else {
                return true;
            }
    ...
    return __awaiter(this, void 0, void 0, function* () {
        if (projectLanguage === constants_1.ProjectLanguage.JavaScript) {
            ...
            if (localProjectRuntime === constants_1.ProjectRuntime.v2) {
                ...
                const oldFuncNodeDebugEnvVar = JavaScriptProjectCreator_1.funcNodeDebugEnvVar.replace(/__/g, ':'); // Also check against an old version of the env var that works in most (but not all) cases
                if (!rawTasksData.includes(JavaScriptProjectCreator_1.funcNodeDebugEnvVar) && !rawTasksData.includes(oldFuncNodeDebugEnvVar)) {
                    const tasksContent = JSON.parse(rawTasksData);
                    const funcTask = tasksContent.tasks.find((t) => funcHostTask_1.funcHostNameRegEx.test(t.label));
                    if (funcTask) {
...
function verifyPythonVenv(projectLanguage, folderPath, actionContext) {
    return __awaiter(this, void 0, void 0, function* () {
        if (projectLanguage === constants_1.ProjectLanguage.Python) {
            const venvName = ProjectSettings_1.getFuncExtensionSetting(PythonProjectCreator_1.pythonVenvSetting, folderPath);
            if (venvName && !(yield fse.pathExists(path.join(folderPath, venvName)))) {
                ...
                if (ProjectSettings_1.getFuncExtensionSetting(settingKey)) {
                    ...
                    const message = localize_1.localize('uninitializedWarning', 'Failed to find Python virtual environment "{0}", which is required to debug and deploy your Azure Functions project.', venvName);
                    ...
                    if (result === createVenv) {
                        yield vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: localize_1.localize('creatingVenv', 'Creating virtual environment...') }, () => __awaiter(this, void 0, void 0, function* () {
                            // create venv
                            yield PythonProjectCreator_1.createVirtualEnviornment(venvName, folderPath);
                            yield PythonProjectCreator_1.makeVenvDebuggable(venvName, folderPath);
                        }));
