...
function createFunction(actionContext, functionAppPath, templateId, functionName, caseSensitiveFunctionSettings, language, runtime) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!templateId) {
            ...
            const templateProvider = yield extensionVariables_1.ext.templateProviderTask;
            const templates = yield templateProvider.getTemplates(language, runtime, functionAppPath, constants_1.TemplateFilter.All, actionContext.properties);
        ...
        for (const setting of template.userPromptedSettings) {
            try {
                ...
                if (!(error instanceof errors_1.SkipForNowError)) { // ignore error if user wants to skip this app setting
                    throw error;
                }
            }
        }
        ...
        if (newFilePath && (yield fse.pathExists(newFilePath))) {
            const newFileUri = vscode_1.Uri.file(newFilePath);
            vscode_1.window.showTextDocument(yield vscode_1.workspace.openTextDocument(newFileUri));
        }
...
function promptForTemplate(functionAppPath, language, runtime, templateFilter, telemetryProperties) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        while (!template) {
            const templateProvider = yield extensionVariables_1.ext.templateProviderTask;
            const templates = yield templateProvider.getTemplates(language, runtime, functionAppPath, templateFilter, telemetryProperties);
