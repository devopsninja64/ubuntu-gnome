...
function getLocalSettings(localSettingsPath, allowOverwrite = false) {
    return __awaiter(this, void 0, void 0, function* () {
        if (yield fse.pathExists(localSettingsPath)) {
            ...
            if (/[^\s]/.test(data)) {
                try {
                    ...
                    if (allowOverwrite) {
                        const message = localize_1.localize('failedToParse', 'Failed to parse local settings: {0}. Overwrite?', vscode_azureextensionui_1.parseError(error).message);
                        const overwriteButton = { title: localize_1.localize('overwrite', 'Overwrite') };
                        // Overwrite is the only button and cancel automatically throws, so no need to check result
                        yield extensionVariables_1.ext.ui.showWarningMessage(message, { modal: true }, overwriteButton, vscode_azureextensionui_1.DialogResponses.cancel);
                    }
                    else {
                        throw error;
                    }
